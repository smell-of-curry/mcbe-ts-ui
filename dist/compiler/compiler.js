...
exports.UICompiler = void 0;
exports.compileUI = compileUI;
exports.quickCompile = quickCompile;
...
class UICompiler {
    ...
    async compile() {
        ...
        return {
            ...
            uiDefs,
            globalVariables: Object.keys(this.globalVariables).length > 0
                ? this.globalVariables
                : undefined,
            errors: this.errors,
    ...
    loadExistingUiDefs() {
        ...
        try {
            ...
            this.existingUiDefs = (parsed.ui_defs || []).filter((entry) => !entry.includes("__generated__"));
        }
        catch {
     ...
     * @private
     */
    findSourceFilesRecursive(dir, files) {
    ...
    async compileFile(sourcePath) {
        ...
        try {
            ...
            const exported = module && typeof module === "object" && "default" in module
                ? module.default
                : module;
            ...
            for (const def of definitions) {
                ...
                if (!("namespace" in def) || !def.namespace) {
                    console.log(`  Skipped: ${relativePath} (invalid definition - missing namespace)`);
                    continue;
                }
     ...
     * @private
     */
    processDefinition(def, sourcePath) {
        const sourceDir = path.resolve(this.config.sourceDir);
        ...
        if (def.isVanillaOverride) {
            ...
            const sourceRelativeDir = path.relative(sourceDir, path.dirname(path.resolve(sourcePath)));
    ...
    generateUiDefs() {
        ...
        for (const file of this.compiledFiles) {
            ...
            if (this.vanillaOverrideFiles.has(file.outputPath))
                continue;
            // Convert Windows paths to forward slashes and make relative
            const entry = file.outputPath.replace(/\\/g, "/");
            if (!uiDefEntries.includes(entry))
                uiDefEntries.push(entry);
        }
        const uiDefs = { ui_defs: uiDefEntries };
        // Write _ui_defs.json
        const uiDefsPath = path.resolve(this.config.uiDefsPath);
        const json = this.config.prettyPrint
 ...
 * ```
 */
async function compileUI(config = {}) {
